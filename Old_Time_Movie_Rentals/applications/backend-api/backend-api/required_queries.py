...
def query1():
    return "use CASE03_OTMR; SELECT CUSTOMER.CustomerName FROM CUSTOMER, MOVIERENTAL WHERE MOVIERENTAL.CustomerID = CUSTOMER.CustomerID;"

# List all the customers who lie in UpTown. List their name and address sorted by name
def query2():
    return "use CASE03_OTMR; SELECT CustomerName, CustomerStreet, CustomerState, CustomerZIP FROM CUSTOMER WHERE CustomerCity LIKE 'UpTown' ORDER BY CustomerName;"

# List the total payment received by each employee, sorted by employee number
def query3():
    return "use CASE03_OTMR; SELECT EMPLOYEE.EmployeeName, EMPLOYEE.EmployeeSIN, TOTAL_PAYMENT.PaymentAmount FROM EMPLOYEE JOIN ( SELECT EmployeeSIN, SUM(PaymentAmount) AS PaymentAmount FROM PAYMENT GROUP BY EmployeeSIN AS TOTAL_PAYMENT ON TOTAL_PAYMENT.EmployeeSIN = EMPLOYEE.EmployeeSIN ORDER BY EmployeeSIN;"

# List the total number of movies rented out by each store, grouped and sorted by store ID
def query4():
    return "use CASE03_OTMR; SELECT MOVIERENTAL.StoreID, COUNT(MOVIERENTAL.StoreID) AS NumberOfRentedOutMovies, STORE.StoreName FROM MOVIERENTAL, STORE WHERE MOVIERENTAL.StoreID = STORE.StoreID GROUP BY StoreID;"

# List all the movies never rented out in each store, and grouped by store ID ans sorted by movie ID
def query5():
    return "use CASE03_OTMR; SELECT MEDIA.MovieID, MEDIA.StoreID, MOVIE.MovieTitle FROM MEDIA, MOVIE WHERE MEDIA.MovieID = MOVIE.MovieID AND MEDIA.MovieID NOT IN (SELECT MOVIERENTAL.MovieID FROM MOVIERENTAL) GROUP BY StoreID, MovieID;"
